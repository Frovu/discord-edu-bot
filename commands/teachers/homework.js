...
module.exports = {
    ...
    exec: async function(message) {
        ...
        // search teacher
        const namePart = args.indexOf('/')?args.slice(0, args.indexOf('/')).trim():args;
        ...
        const subj = teachers.chooseSubj(t, message);
        if(!subj) return;
        const comment = args.indexOf('/')?args.slice(args.indexOf('/')+1).trim():'';
        // parse attachments
